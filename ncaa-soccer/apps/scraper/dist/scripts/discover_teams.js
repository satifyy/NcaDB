...
const SOCIAL_OR_VENDOR = /twitter|facebook|instagram|youtube|tiktok|ticketmaster|seatgeek|evenue|espn|sidearm|shopify|x\.com|google|apple|paciolan|learfield|hostedstats|statbroadcast|wmt\.|prestosports/i;
function slug(value) {
...
function matchKeys(name) {
    let base = (0, parsers_1.cleanTeamName)(name).name.toLowerCase();
...
function harvestFromHtml(html, ownHost) {
    const found = new Map();
    ...
    while ((match = anchor.exec(html))) {
        ...
        if (!host || host === ownHost || SOCIAL_OR_VENDOR.test(host))
            continue;
        const name = (0, parsers_1.cleanTeamName)(rawTitle.replace(/&#39;/g, "'").replace(/&amp;/g, '&')).name;
        if (!name)
            continue;
...
async function harvestFromTeam(team, season) {
    const ownHost = new URL(team.schedule_url).hostname.replace(/^www\./, '');
    if (team.platform_guess === 'wmt') {
        const found = new Map();
        try {
            const sportSlug = (0, parsers_1.sportSlugFromScheduleUrl)(team.schedule_url);
            if (!sportSlug)
                return found;
            const events = await new parsers_1.WmtClient(team.schedule_url).fetchSeasonEvents(sportSlug, (0, parsers_1.seasonNameCandidates)(season));
            for (const event of events) {
                const url = event.opponent?.website_url;
                const name = (0, parsers_1.cleanTeamName)(event.opponent_school_name || event.opponent_name).name;
                if (!url || !name)
                    continue;
                try {
...
async function probeDomain(host, schoolName, season) {
    for (const sportSlug of SPORT_SLUGS) {
        ...
        if (!page.ok || !page.body)
            continue;
        if (!/soccer/i.test(page.body.slice(0, 200000)))
            continue;
        if (page.body.includes('/website-api')) {
            try {
                const events = await new parsers_1.WmtClient(scheduleUrl).fetchSeasonEvents(sportSlug, (0, parsers_1.seasonNameCandidates)(season));
                if (events.length > 0) {
        ...
        if (/soccer/i.test(title) && !/404/.test(title)) {
            ...
            const games = (body.match(/data-test-id="s-game-card-standard__root"|<li[^>]+class="[^"]*sidearm-schedule-game\b/g) || []).length;
...
function conferenceFile(conference) {
...
async function main() {
    ...
    const rosters = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), rostersArg), 'utf8'));
    ...
    for (const file of fs.readdirSync(teamsDir).filter(f => f.endsWith('_teams.json'))) {
        try {
            for (const team of (0, teams_1.loadTeams)(path.join(teamsDir, file))) {
    ...
    const written = [];
    const unresolved = () => wanted.flatMap(([, schools]) => schools).filter(s => !matchKeys(s).some(k => domains.has(k)));
    ...
    for (let round = 1; round <= 3 && unresolved().length > 0; round++) {
        ...
        for (const batch of harvested)
            added += absorb(domains, batch);
    ...
    for (const [conference, schools] of wanted) {
        ...
        const resolved = await mapLimit(schools, CONCURRENCY, async (school) => {
            ...
            if (!host)
                return { school, host: null, probe: null };
...
function mergeWithExisting(outPath, discovered) {
    if (!fs.existsSync(outPath))
        return discovered;
    const byId = new Map(discovered.map(team => [team.team_id, team]));
    try {
        for (const team of JSON.parse(fs.readFileSync(outPath, 'utf8'))) {
            if (team?.team_id && team.schedule_url && !byId.has(team.team_id))
 ...
 */
function mergeInventories(dir, justWritten) {
    const byId = new Map();
    for (const team of justWritten)
        byId.set(team.team_id, team);
    for (const file of fs.readdirSync(dir)) {
        if (!file.endsWith('_teams.json') || file === COMBINED_FILE || file === 'test_teams.json')
            continue;
        try {
            for (const team of (0, teams_1.loadTeams)(path.join(dir, file))) {
                if (!byId.has(team.team_id))
                    byId.set(team.team_id, team);
            }
        }
        catch {
...
//# sourceMappingURL=discover_teams.js.map