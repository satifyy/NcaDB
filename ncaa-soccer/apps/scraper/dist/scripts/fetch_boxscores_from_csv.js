 ...
 */
const wmtBoxParser = new parsers_1.WmtBoxScoreParser();
let wmtHosts = new Set();
let teamNameResolver;
function hostOf(url) {
    try {
 ...
 */
function resolveTeamLabels(game, labels) {
    const canonical = (name) => teamNameResolver ? teamNameResolver.canonical(name) : name;
    ...
    if (unassigned.length === 1 && remaining.length === 1) {
        ...
        if (Math.max(straight, crossed) > 0 && straight !== crossed) {
            ...
            for (const label of unassigned)
                assignment.set(label, canonical(label));
        }
    }
    else {
...
function affinity(label, team) {
    ...
    if (initials.length > 1 && l.startsWith(initials))
        return 20 + initials.length;
    if (initials.length > 1 && initials.startsWith(l))
        return 15 + l.length;
    return 0;
}
/** Canonicalises `team_id` and rebuilds `player_key`, which embeds it. */
function toPlayerRows(game, parsed) {
...
async function processWmtGame(game) {
    try {
...
function startStatusTicker(status) {
 ...
 */
async function processGame(browser, game, rawDir, boxParser, opts) {
    const primary = await processGameOnce(browser, game, rawDir, boxParser, opts);
    if (primary.success || !game.boxscore_url_alt)
        return primary;
    logDebug(`FALLBACK [${game.game_id}] Primary box score empty, trying ${game.boxscore_url_alt}`);
    const fallback = await processGameOnce(browser, { ...game, boxscore_url: game.boxscore_url_alt, boxscore_url_alt: '' }, rawDir, boxParser, opts);
    if (fallback.success) {
        logDebug(`FALLBACK OK [${game.game_id}] Recovered ${fallback.rows.length} stats from the alternate source`);
    }
    return fallback;
}
async function processGameOnce(browser, game, rawDir, boxParser, opts) {
    ...
    try {
        ...
        if (res.playerStats.length > 0) {
            ...
            const meta = [];
            if (wmtUrl)
                meta.push(`wmt=${wmtUrl}`);
            if (pdfUrls.length)
                meta.push(`pdfs=${pdfUrls.join('|')}`);
...
async function main() {
    ...
    const teams = (0, teams_1.loadAllTeams)();
    teamNameResolver = (0, teams_1.buildTeamNameResolver)(teams);
    wmtHosts = new Set(teams
 ...
 */
function carryForwardMissingGames(outPath, freshRows, inScope) {
    if (!fs.existsSync(outPath))
        return { rows: [], games: 0 };
    let previous;
    try {
        previous = (0, sync_1.parse)(fs.readFileSync(outPath, 'utf8'), { columns: true, skip_empty_lines: true });
    }
    catch (e) {
    ...
    const rows = previous.filter(r => r.game_id && inScope.has(r.game_id) && !scrapedThisRun.has(r.game_id));
