 ...
 * keeps it.
 */
function normalizeRow(row, resolver) {
    const home = (0, parsers_1.cleanTeamName)(row.home_team_name);
    const away = (0, parsers_1.cleanTeamName)(row.away_team_name);
    const homeName = resolver.canonical(home.name);
    const awayName = resolver.canonical(away.name);
    if (!homeName || !awayName || !row.date)
        return row;
    return {
        ...row,
        ...
        dedupe_key: (0, parsers_1.makeDedupeKey)(row.date, homeName, awayName)
    };
}
/**
 ...
 */
function sidearmScheduleUrl(scheduleUrl, seasonYear) {
    if (seasonYear === new Date().getFullYear())
        return scheduleUrl;
    if (/\/(19|20)\d{2}\/?$/.test(scheduleUrl))
        return scheduleUrl;
 ...
 */
async function processWmtSchool(team, resolver) {
    ...
    const seasonNames = (0, parsers_1.seasonNameCandidates)(SEASON_YEAR);
    const client = new parsers_1.WmtClient(team.schedule_url);
    const events = await client.fetchSeasonEvents(sportSlug, seasonNames);
    const games = new parsers_1.WmtParser().parseEvents(events, {
 ...
 */
async function processWmtWordpressSchool(team, resolver) {
    ...
    const games = await new parsers_1.WmtWordpressParser().parseSchedule(await response.text(), {
        teamName: team.name_canonical,
        baseUrl: team.schedule_url,
        sourceUrl: url,
        nameResolver: resolver,
...
async function processSchool(browser, team, resolver) {
    ...
    // });
    try {
        const response = await retryWithBackoff(() => page.goto(scheduleUrl, {
...
async function main() {
    ...
    const teams = (0, teams_1.loadTeams)(TEAMS_JSON_PATH);
    const resolver = (0, teams_1.buildTeamNameResolver)(teams);
    ...
    for (let i = 0; i < teams.length; i += CONCURRENCY) {
        ...
        const promises = batch.map(team => retryWithBackoff(() => processSchool(browser, team, resolver), 2, // max 2 retries per team
    ...
    const placeholders = allGames.filter(g => /unknown/i.test(`${g.home_team_name} ${g.away_team_name}`));
