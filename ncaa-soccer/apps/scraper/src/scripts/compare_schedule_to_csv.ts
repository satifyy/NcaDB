...
const compare = () => {
    ...
    (async () => {
        ...
        for (const g of parsed) {
            ...
            if (csvBox !== parsedBox && parsedBox) diffs.push(`boxscore mismatch csv='${csvBox}' parsed='${parsedBox}'`);
            const ps = `${g.home_score ?? ''}`;
            const pa = `${g.away_score ?? ''}`;
            const cs = `${csvRow.home_score || ''}`;
            const ca = `${csvRow.away_score || ''}`;
            if (ps !== cs || pa !== ca) diffs.push(`score mismatch csv=${cs}-${ca} parsed=${ps}-${pa}`);
            if (csvRow.status !== g.status) diffs.push(`status mismatch csv=${csvRow.status} parsed=${g.status}`);
            if (diffs.length === 0) {
