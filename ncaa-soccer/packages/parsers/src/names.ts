...
const QUALIFIER_RE =
    /\s*\((?:[^()]*\b(?:exhibition|scrimmage|friendly|round|rd\.?|final|finals|semifinal|quarterfinal|tournament|championship|classic|invitational|acc|ncaa|naia|njcaa|usl|mls|mlsnext|nwsl|uslc|usl\s*1|play-?in)\b[^()]*)\)\s*$/i;
 ...
 */
const RANK_RE =
    /^\s*(?:no\.?\s*|#\s*|\(\s*)(\d{1,2})\s*(?:\/\s*[A-Za-z0-9]+)?\s*\)?\s+(?:seed\s+)?/i;
...
export function cleanTeamName(raw: string | null | undefined): CleanedName {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (!qualifier) break;
        hadQualifier = true;
        if (/exhibition|scrimmage|friendly/i.test(qualifier[0])) exhibition = true;
...
export class TeamNameResolver {
    ...
    constructor(canonicalToAliases: Record<string, string[]> = {}) {
        ...
        for (const [key, canonical] of derived) {
            if (!this.aliases.has(key)) this.aliases.set(key, canonical);
        }
    }
