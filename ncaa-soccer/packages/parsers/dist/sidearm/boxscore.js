            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (typeof item === 'object' && item !== null && !Array.isArray(item)) {
                    ...
                    if ('players' in item && 'id' in item && 'name' in item) {
                        ...
                        if (typeof playersVal === 'number')
                            playersVal = data[playersVal]; // This gets the array (or index to array)
                        ...
                        if (Array.isArray(playersVal)) {
                            ...
                            playersVal.forEach((playerIndex) => {
                                // Resolve player object
                                let playerObj = playerIndex;
                                if (typeof playerObj === 'number')
                                ...
                                if (typeof playerObj === 'object' && playerObj) {
                                    ...
                                    if (typeof shotsObj === 'number')
                                        shotsObj = data[shotsObj];
                                    if (shotsObj) {
                                    ...
                                    if (typeof goalieObj === 'number')
                                        goalieObj = data[goalieObj];
                                    if (goalieObj) {
                                    ...
                                    if (minutes === 0 && goalieMinutes > 0)
                                        minutes = goalieMinutes;
                                    const stat = {
                                        game_id: gameId,
                                        team_id: teamName,
                                        player_name: name,
